...
One_Days_A = Today - datetime.timedelta(days=1)
TF_days_A = Today - datetime.timedelta(days=35)
Four_Days_B = Today + datetime.timedelta(days=4)

Today_S = Today.strftime("%Y-%m-%d")
One_S = One_Days_A.strftime("%Y-%m-%d")
TF_S = TF_days_A.strftime("%Y-%m-%d")
Four_S = Four_Days_B.strftime("%Y-%m-%d")
...
def Register(request):
    if request.method == 'POST':
        ...
        Ok=1
        L = []
        if Register.objects.filter(username=Context['username']).exists():
        ...
        Data = Register(fullname=Context['fullName'],
                        username=Context['username'],
                        email=Context['email'],
                        phone_number=Context['phoneNumber'],
                        passward=Context['password'],
                        confirmpassward=Context['confirmPassword'])
...
def login(request):
    if request.method == 'POST':
        ...
        Ok=1
        L = []
        if not Register.objects.filter(username=Context['username']).exists():
            L.append("User Not Exist !!!")
            Ok=0
        else:
            Password = Register.objects.get(username=Context['username']).passward
            if Password != Context['password']:
                L.append("Password Does Not Match !!!")
                Ok=0

        if Ok:
            return JsonResponse({'message' : 'Login Successfully','ok': Ok})
        else:
            return JsonResponse({'message': L[0]})
